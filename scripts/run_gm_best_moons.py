...
def run_block(tag, X, y, seed, outdir, imbalance, budget_match):
    ...
    gm_cfg = GMConfig(
        ...
        alpha=0.1,                     # mild diffusion
        beta=1.5,                      # sharp RBF falloff
    ...
    results_smooth.append(dict(tag=tag, method="GM",             smooth=sm_gm))
    results_smooth.append(dict(tag=tag, method="GM-flat",        smooth=sm_gm_flat))
...
def run_suite(dataset: str, outdir: str, seed: int, imbalance: float, budget_match: bool):
    ...
    if dataset == "moons":
        ...
        X, y = gen_circles(n_samples=2000, seed=seed, factor=0.1, noise=0.25)
    else:
