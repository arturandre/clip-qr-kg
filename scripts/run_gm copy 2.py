...
# assumes you already have these helpers defined elsewhere:
# - l2n, prototype_purity, reliability_full
# - margins (on prototypes)
# - (optional) silhouette_normalized, dispersion, instability
# We’ll provide fast, subsampled versions below.
...
class GMConfig:
    ...
    disable_instability: bool = True               # instability is expensive; off by default
    instab_sigma: float = 0.05                     # if enabled
    instab_trials: int = 1                         # if enabled

    # Diffusion: 0 = closed form (inverse); >0 = T iterative steps (no inverse)
    diffusion_iters: int = 20

# ---------------------------
# Fast reliability (subsampled)
# ---------------------------

def _cluster_subsample_indices(assign: np.ndarray, cap: int, seed: int = 123):
    """Return list of index arrays (one per cluster) with ≤cap items each."""
    K = int(assign.max()) + 1
...
def _silhouette_approx(X: np.ndarray, assign: np.ndarray, subs, K: int) -> np.ndarray:
    """Silhouette on subsamples only (fast, good enough for reliability)."""
    s_c = np.zeros(K, dtype=np.float32)
    for c in range(K):
        I = subs[c]
        if I.size <= 1: s_c[c] = 0.5; continue
        Xi = X[I]
...
def _instability_sub(X: np.ndarray, assign: np.ndarray, P: np.ndarray,
                     subs, sigma=0.05, trials=1) -> np.ndarray:
    # compute instability only on union of subsamples
    I_all = np.concatenate([I for I in subs if I.size > 0]) if any(len(I)>0 for I in subs) else np.arange(min(len(X), 1024))
    if I_all.size == 0:
        return np.zeros(len(P), dtype=np.float32)
    Xs = X[I_all]
    K = len(P)
    changed = np.zeros(K, dtype=np.float32)
    counts = np.zeros(K, dtype=np.float32)
    nn = NearestNeighbors(n_neighbors=1).fit(P)
    for _ in range(trials):
        Xd = Xs + np.random.normal(scale=sigma, size=Xs.shape).astype(Xs.dtype)
        nn_idx = nn.kneighbors(Xd, return_distance=False).ravel()
        for i, i_global in enumerate(I_all):
            c0 = assign[i_global]
            counts[c0] += 1.0
            if nn_idx[i] != c0:
                changed[c0] += 1.0
...
class GraphMemory:
    ...
    def __init__(self, cfg: GMConfig):
        self.cfg = cfg
        self.P = None      # [K, d] prototypes (float32)
        self.yc = None     # [K] dominant class per prototype (int)
        self.r  = None     # [K] reliability per prototype (float32)
        self.S  = None     # [K, K] row-stochastic transitions (float32)
        self._M = None     # cached (I - alpha S)^-1 if diffusion_iters == 0
        self._nnP = None   # NearestNeighbors over P for fast attachment
    ...
    def fit(self, X, y):
        ...
        # --- Prototype labels & reliability (fast, subsampled)
        nC = int(y.max()) + 1
        yc, pi = prototype_purity(assign, y, K, nC)     # (cheap) uses all points
        ...
        if self.cfg.disable_instability:
            ...
            rho = _instability_sub(X, assign, P, subs,
                                   sigma=self.cfg.instab_sigma,
                                   trials=self.cfg.instab_trials).astype(np.float32)

        r = reliability_full(s, m, v, rho, pi).astype(np.float32)

        # --- Prototype graph (KNN + RBF), row-stochastic
        ...
        S = (A / D).astype(np.float32)

        self.P, self.yc, self.r, self.S = Pn.astype(np.float32), yc.astype(np.int32), r, S
        self._nnP = NearestNeighbors(n_neighbors=min(self.cfg.attach_k, K)).fit(self.P)

        # Optional: precompute inverse if using closed form
    ...
    def _build_Z0(self, Xq: np.ndarray) -> np.ndarray:
        """Return Z0 of shape [K, Q] for all queries at once (reliability-weighted RBF)."""
        Xq = np.asarray(Xq, dtype=np.float32 if self.cfg.dtype == "float32" else np.float64)
        Q = len(Xq); K = len(self.P)
    ...
    def predict_proba(self, Xq, n_classes: int) -> np.ndarray:
        ...
        if self.cfg.diffusion_iters == 0:
            # Closed form: Z = (1-α) * M @ Z0
            Z = (1.0 - self.cfg.alpha) * (self._M @ Z0)         # [K, Q]
        else:
            # Iterative: Z_{t+1} = α S Z_t + (1-α) Z0
            Z = Z0.copy()
            a = self.cfg.alpha
            for _ in range(self.cfg.diffusion_iters):
                Z = a * (self.S @ Z) + (1.0 - a) * Z0

        # Aggregate to class probabilities
        Q = Z.shape[1]
...
def probs_labels_spreading_inductive(Xtr, ytr, Xte, n_neighbors=10, alpha=0.2):
    """
    Inductive use of sklearn LabelSpreading:
      - fit ONLY on (Xtr, ytr)
      - predict_proba on Xte
    """
    ls = LabelSpreading(kernel="knn", n_neighbors=n_neighbors, alpha=alpha, max_iter=30)
    ls.fit(Xtr, ytr)  # no unlabeled points from test here
    return ls.predict_proba(Xte)


def lp_labels_on_grid_inductive(Xtr, ytr, Z, n_neighbors=10, alpha=0.2):
    return probs_labels_spreading_inductive(Xtr, ytr, Z, n_neighbors=n_neighbors, alpha=alpha).argmax(1)
...
def probs_labels_spreading_transductive(XL, yL, XU, n_neighbors=10, alpha=0.2):
    ...
    lp.fit(X_all, y_all)
    return lp.label_distributions_[-len(XU):]
...
def run_block(tag, Xtr, Xte, ytr, yte, Xcal, ycal, seed, outdir, imbalance, budget_match):
    ...
    # LabelSpreading (inductive: fit on train only, predict on test)
    t0 = perf_counter()
    ls_model_fit_start = t0
    # Fit timing
    ls = LabelSpreading(kernel="knn", n_neighbors=10, alpha=0.2, max_iter=30)
    ls.fit(Xtr, ytr)
    tfit_lps = perf_counter() - ls_model_fit_start
    # Predict timing
    t1 = perf_counter()
    P_tst_ls = ls.predict_proba(Xte)
    tpred_lps = perf_counter() - t1
    ...
    cal_rows = [
        ...
        dict(tag=tag, method="LabelSpreading",  **all_metrics(P_tst_ls, yte)),  # <- inductive probs
    ...
    plot_decision(
        axs[0,2],
        lambda Z: lp_labels_on_grid_inductive(Xtr, ytr, Z, n_neighbors=10, alpha=0.2),
        Xtr, ytr, Xte, yte, f"LabelSpreading (inductive)"
    )
    plot_decision(axs[1,0], lambda Z: gm.predict(Z, nC),       Xtr, ytr, Xte, yte, f"GM ({acc_gm:.3f})")
    plot_decision(axs[1,1], lambda Z: gm_flat.predict(Z, nC),  Xtr, ytr, Xte, yte, f"GM-flat ({acc_gmf:.3f})")
    plot_decision(axs[1,2], lambda Z: gm_centroid(Xtr, ytr).predict(Z, nC), Xtr, ytr, Xte, yte, f"GM-centroid ({acc_gmc:.3f})")
