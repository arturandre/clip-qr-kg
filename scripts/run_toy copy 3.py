...
def time_lp_fit_predict(lp_model, Xtr, ytr, Xte, yte):
    # semi-supervised: fit on Xtr+Xte com rótulos -1 no teste
    X_all = np.r_[Xtr, Xte]
    y_all = np.r_[ytr, -np.ones_like(yte)]
    t0 = perf_counter()
    lp_model.fit(X_all, y_all)
    t_fit = perf_counter() - t0
    # sklearn guarda transduction_ para todos; pegamos a cauda referente ao teste
    t0 = perf_counter()
...
def run_suite(dataset: str, outdir: str, seed: int):
    ...
    # 2-class sweep
    console.print(Panel.fit(f"[bold]Dataset:[/bold] {dataset} — [cyan]2-class sweep[/cyan]"))
    for idx, setting in enumerate(tqdm(two_class_settings, desc="2-class configs", leave=False)):
        if dataset == "blobs":
        ...
        y_lp,  tfit_lp,  tpred_lp  = time_lp_fit_predict(baseline_lp(n_neighbors=10), Xtr, ytr, Xte, yte)

        # GM + degenerates (timed)
        Kproto = int(min(32, max(8, len(Xtr)//8)))
        gm_cfg = GMConfig(K=Kproto, knn_graph=10, attach_k=3, alpha=0.6, beta=1.0)
        ...
        plot_decision(axs[0,2], lambda Z: baseline_lp(n_neighbors=10).fit(np.r_[Xtr, Xte], np.r_[ytr, -np.ones_like(yte)]).predict(Z), Xtr, ytr, Xte, yte, f"LP ({acc_lp:.3f})", nC)
    ...
    # Multi-class sweep
    console.print(Panel.fit(f"[bold]Dataset:[/bold] {dataset} — [cyan]Multiclass sweep[/cyan]"))
    for idx, nlev in enumerate(tqdm(multiclass_settings, desc="multiclass configs", leave=False)):
        if dataset == "blobs":
        ...
        y_lin, tfit_lin, tpred_lin = time_fit_predict(baseline_linear(), Xtr, ytr, Xte)
        y_knn, tfit_knn, tpred_knn = time_fit_predict(baseline_knn(k=10), Xtr, ytr, Xte)
        y_lp,  tfit_lp,  tpred_lp  = time_lp_fit_predict(baseline_lp(n_neighbors=15), Xtr, ytr, Xte, yte)

        # GM timed
        Kproto = int(min(8*nC, max(2*nC, len(Xtr)//12)))
        gm_cfg = GMConfig(K=Kproto, knn_graph=12, attach_k=3, alpha=0.6, beta=1.0)
        gm, y_gm, tfit_gm, tpred_gm = time_gm_fit_predict(gm_cfg, Xtr, ytr, Xte, nC)
        ...
        plot_decision(axs[0,0], lambda Z: baseline_linear().fit(Xtr, ytr).predict(Z), Xtr, ytr, Xte, yte, f"Linear ({acc_lin:.3f})", nC)
        plot_decision(axs[0,1], lambda Z: baseline_knn(k=10).fit(Xtr, ytr).predict(Z), Xtr, ytr, Xte, yte, f"kNN ({acc_knn:.3f})", nC)
        plot_decision(axs[0,2], lambda Z: baseline_lp(n_neighbors=15).fit(np.r_[Xtr, Xte], np.r_[ytr, -np.ones_like(yte)]).predict(Z), Xtr, ytr, Xte, yte, f"LP ({acc_lp:.3f})", nC)
        plot_decision(axs[1,0], lambda Z: gm.predict(Z, nC),       Xtr, ytr, Xte, yte, f"GM ({acc_gm:.3f})", nC)
        plot_decision(axs[1,1], lambda Z: gm_flat.predict(Z, nC),  Xtr, ytr, Xte, yte, f"GM-flat ({acc_gmflat:.3f})", nC)
        plot_decision(axs[1,2], lambda Z: gm_cent.predict(Z, nC),  Xtr, ytr, Xte, yte, f"GM-centroid ({acc_gmcent:.3f})", nC)
        plt.suptitle(tag, fontsize=11)
...
def main():
    ...
    args = ap.parse_args()
    with section(f"Running suite: {args.dataset}"):
